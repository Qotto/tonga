...
from aioevent.models.store_record.base import BaseStoreRecordHandler
...
class StoreRecordHandler(BaseStoreRecordHandler):
    ...
    async def local_store_handler(self, store_record: StoreRecord, group_id: str, tp: TopicPartition,
                                  offset: int) -> None:
        ...
        self._store_builder.update_metadata_from_local_store(tp, offset)

    async def global_store_handler(self, store_record: StoreRecord, group_id: str, tp: TopicPartition,
                                   offset: int) -> None:
        # Set or delete from global store
        if store_record.type == 'set':
            self._store_builder.set_from_global_store(store_record.key, store_record.value)
        elif store_record.type == 'del':
            self._store_builder.delete_from_global_store(store_record.key)
        else:
            raise KtableUnknownType(f'Global state type: {store_record.type} is unknown', 500)
        # Update metadata from global store
        self._store_builder.update_metadata_from_global_store(tp, offset)
